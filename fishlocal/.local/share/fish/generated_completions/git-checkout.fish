...
complete -c git-checkout -l orphan -d 'Create a new unborn branch, named <new-branch>, started from <start-point> an…'
...
complete -c git-checkout -l rebase -d 'ours and theirs may appear swapped;'
complete -c git-checkout -s a -d 'to make the root commit'
complete -c git-checkout -o rf -d ' from the top level of the working tree'
