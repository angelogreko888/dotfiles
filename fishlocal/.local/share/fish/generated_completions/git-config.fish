...
complete -c git-config -l append -d 'Adds a new line to the option without altering any existing values'
complete -c git-config -l comment -d 'Append a comment at the end of new or modified lines. sp . if n \\{\\'
complete -c git-config -l all -d 'With get, return all values for a multi-valued key'
complete -c git-config -l regexp -d 'With get, interpret the name as a regular expression'
complete -c git-config -l url -d 'When given a two-part <name> as <section>. <key>, the value for <section>'
...
complete -c git-config -l name-only -d 'Output only the names of config variables for list or get'
...
complete -c git-config -l default -d 'When using get, and the requested variable is not found, behave as if <value>…'
complete -c git-config -l value -d 'will replace all config options whose values match the given pattern.  unset'
complete -c git-config -l '<type>'
complete -c git-config -s l -l list
complete -c git-config -l get
complete -c git-config -l get-all
complete -c git-config -l get-regexp
complete -c git-config -l show-names
complete -c git-config -l get-urlmatch
complete -c git-config -l get-color
complete -c git-config -l add
complete -c git-config -l unset
complete -c git-config -l unset-all
complete -c git-config -l rename-section
complete -c git-config -l remove-section
complete -c git-config -s e -l edit
complete -c git-config -s c -d 'option With the exception of command, each scope corresponds to a command lin…'
complete -c git-config -l decorate -d 'output) is set to be painted with bold or some other attribute'
complete -c git-config -l ignore-errors -d 'option of git-add(1).  add'
complete -c git-config -l no-refresh -d 'option.  resolveConflict'
complete -c git-config -l no-ahead-behind -d 'is given.  statusHints'
complete -c git-config -s u -d 'option.  submoduleAlternateErrorStrategyDie'
complete -c git-config -l init -d 'was not run.  suggestDetachingHead'
complete -c git-config -l detach -d 'option.  updateSparsePath'
complete -c git-config -s p -d 'to force pagination.  For example, loud-rebase ='
complete -c git-config -l not -d 'ORIG_HEAD, the invocation git new is equivalent to running the shell command …'
complete -c git-config -l show-prefix -d 'from the original current directory.  See git-rev-parse(1).  oc o 2. 3'
complete -c git-config -l keep-cr
...
complete -c git-config -l whitespace -d 'option.  See git-apply(1).  attr. tree'
complete -c git-config -l attr-source -d 'command line option are used, this configuration variable has no effect'
...
complete -c git-config -l edit-description -d 'browser. <tool>. cmd'
complete -c git-config -s w -d 'option in git-help(1)) or a working repository in gitweb (see git-instaweb(1))'
complete -c git-config -l bundle-uri -d option
...
complete -c git-config -l reject-shallow -d 'option on the command line.  See git-clone(1).  clone. filterSubmodules'
...
complete -c git-config -l color-by-age -d 'depending upon the age of the line'
complete -c git-config -l color-lines -d 'color. branch'
complete -c git-config -l color -d 'color. diff. <slot>'
...
complete -c git-config -l author
complete -c git-config -l committer -d selected
complete -c git-config -s i -d 'column. ui for details.  column. status'
...
complete -c git-config -l format
complete -c git-config -l work-tree -d 'command-line option'
complete -c git-config -s S -d 'LESS=FRX less'
complete -c git-config -o +F -d 'will deactivate the F option specified by the environment from the command-li…'
...
complete -c git-config -l refresh -d 'to update the cached stat information for paths whose contents in the work tr…'
complete -c git-config -l dirstat -d 'parameters specifying the default behavior of the'
complete -c git-config -l '*stat' -d files
complete -c git-config -l stat -d 'output.  If set, applies to all commands generating'
complete -c git-config -s U -d 'option.  diff. interHunkContext'
complete -c git-config -l inter-hunk-context -d 'command line option.  diff. external'
complete -c git-config -l ignore-submodules -d 'git diff Porcelain, and not lower level diff commands such as git diff-files'
...
complete -c git-config -s d
complete -c git-config -l relative-paths -d 'option or with the worktree. useRelativePaths config set to true'
complete -c git-config -l prune -d 'option was given on the command line.  See also remote. <name>'
complete -c git-config -l no-all -d 'or by explicitly specifying one or more remote(s) to fetch from'
...
complete -c git-config -l graph -d 'fetch. bundleURI'
...
complete -c git-config -o p0 -d 'option.  fsck. <msg-id>'
...
complete -c git-config -l auto -d 'will pack them'
complete -c git-config -l keep-largest-pack -d 'except that all non-cruft packs that meet the threshold are kept, not just th…'
complete -c git-config -l max-cruft-size
complete -c git-config -l cruft -d 'git gc runs concurrently with another process writing to the repository; see …'
complete -c git-config -l amend -d 'or git rebase and are the commits prior to the amend or rebase occurring'
complete -c git-config -l filter-to -d 'option of git-repack(1) and the objects/info/alternates section of gitreposit…'
...
complete -c git-config -l verify -d '$signature'
complete -c git-config -o bsau -d '$key" is fed with the contents to be signed, and the program is expected to s…'
...
complete -c git-config -l no-index -d 'if git grep is executed outside of a git repository.  Defaults to false.  gui'
...
complete -c git-config -l object-format -d 'in git-init(1)'
complete -c git-config -l ref-format -d 'in git-init(1)'
...
complete -c git-config -l schedule -d 'commands, provided no'
...
complete -c git-config -l help -d 'mergetool. meld'
...
complete -c git-config -l pretty -d 'to be equivalent to running git log "--pretty=format:* %H %s"'
complete -c git-config -l quiet -d 'when fetching additional objects for a partial clone.  protocol. allow'
...
complete -c git-config -l autosquash -d 'option of git-rebase(1) by default for interactive mode'
complete -c git-config -l no-autosquash -d 'option.  rebase. autoStash'
...
complete -c git-config -l refetch -d 'option of git-fetch(1).  remote. <name>. serverOption'
complete -c git-config -l server-option -d 'command line arguments.  remote. <name>. followRemoteHEAD'
...
complete -c git-config -s a -d 'is run)'
...
complete -c git-config -l mailmap
complete -c git-config -l no-mailmap -d 'sendemail. mailmap. file'
...
complete -c git-config -l remote -d 'gitmodules file.  See git-submodule(1) and gitmodules(5) for details'
