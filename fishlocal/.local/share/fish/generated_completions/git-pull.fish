...
complete -c git-pull -l all -d 'Fetch all remotes, except for the ones that has the remote. <name>'
...
complete -c git-pull -l rebase-merges -d 'so that the local merge commits are included in the rebase (see git-rebase(1)…'
