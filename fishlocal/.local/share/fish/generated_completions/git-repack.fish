...
complete -c git-repack -l max-cruft-size -d 'Repack cruft objects into packs as large as <n> bytes before creating new pac…'
...
complete -c git-repack -l filter -d 'Remove objects matching the filter specification from the resulting packfile …'
complete -c git-repack -l filter-to -d 'Write the pack containing filtered out objects to the directory <dir>'
...
complete -c git-repack -o 'g<factor>' -l geometric -d 'Arrange resulting pack structure so that each successive pack contains at lea…'
complete -c git-repack -s m -l write-midx -d 'Write a multi-pack index (see git-multi-pack-index(1)) containing the non-red…'
complete -c git-repack -l full
complete -c git-repack -l dangling -d 'shows as dangling'
...
complete -c git-repack -l no-write-bitmap-index -d '(or the repack'
