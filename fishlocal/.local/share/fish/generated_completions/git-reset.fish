...
complete -c git-reset -l staged -d '<pathspec>'
complete -c git-reset -l source -d 'git reset (--patch | -p) [<tree-ish>] [--] [<pathspec>. ]'
complete -c git-reset -s p -d 'is the opposite of git add'
...
complete -c git-reset -s u
complete -c git-reset -s m -d '<commit>, but carries forward unmerged index entries'
...
complete -c git-reset -s c -d 'branch2 start), but nobody is perfect.  T} 3. 	T{ But you can use reset'
complete -c git-reset -l cached -d 'T} 4. 	T{ Next, commit the changes stored in the index.  The'
