...
complete -c git-shortlog -l grep -d 'Limit the commits output to ones with a log message that matches the specifie…'
complete -c git-shortlog -l all-match -d 'Limit the commits output to ones that match all given --grep, instead of ones…'
complete -c git-shortlog -l invert-grep -d 'Limit the commits output to ones with a log message that do not match the pat…'
...
complete -c git-shortlog -l cherry-pick -d 'Omit any commit that introduces the same change as another commit on the "oth…'
...
complete -c git-shortlog -l merge -d 'Show commits touching conflicted paths in the range HEAD'
...
complete -c git-shortlog -l 'no-merges;' -d 'git log'
