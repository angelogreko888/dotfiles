...
complete -c llvm-objcopy -l remove-symbol-prefix -d 'Remove <prefix> from the start of every symbol name'
...
complete -c llvm-objcopy -l change-section-lma -d 'Shift LMA of non-zero-sized segments by <val>'
...
complete -c llvm-objcopy -l compress-sections -d 'Compress or decompress sections matched by <section> using the specified form…'
...
complete -c llvm-objcopy -l gap-fill -d 'For binary outputs, fill the gaps between sections with <value> instead of ze…'
...
complete -c llvm-objcopy -l keep-global-symbol -s G -d 'Mark all symbols local in the output, except for symbols with the name <symbo…'
complete -c llvm-objcopy -l keep-global-symbols -d 'Mark all symbols local in the output, except for symbols named in the file <f…'
...
complete -c llvm-objcopy -l localize-hidden -d 'Mark all symbols with hidden or internal visibility local in the output'
...
complete -c llvm-objcopy -l no-verify-note-sections -d 'When adding note sections, do not verify if the section format is valid'
complete -c llvm-objcopy -l output-target -s O -d 'Write the output as the specified format'
complete -c llvm-objcopy -l pad-to -d 'For binary outputs, pad the output to the load address <address> using a valu…'
...
complete -c llvm-objcopy -l set-start -d 'Set the start address of the output to <addr>'
complete -c llvm-objcopy -l set-symbol-visibility -d 'Change the visibility of a symbol to the specified value'
complete -c llvm-objcopy -l set-symbols-visibility -d 'Read a list of symbols from <filename> and change their visibility to the spe…'
complete -c llvm-objcopy -l skip-symbol -d 'Do not change the parameters of symbol <symbol> when executing other options …'
complete -c llvm-objcopy -l skip-symbols -d 'Do not change the parameters of symbols named in the file <filename> when exe…'
...
complete -c llvm-objcopy -l verify-note-sections -d 'When adding note sections, verify if the section format is valid'
complete -c llvm-objcopy -l weaken-symbol -s W -d 'Mark global symbols named <symbol> as weak symbols in the output'
...
complete -c llvm-objcopy -o freebsd -d 'BINARY INPUT AND OUTPUT'
