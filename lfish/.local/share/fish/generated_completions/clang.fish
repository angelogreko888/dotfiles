...
complete -c clang -o mmacos-version-min -d 'When building for macOS, specify the minimum version supported by your applic…'
...
complete -c clang -l print-enabled-extensions -d 'Prints the list of extensions that are enabled for the target specified by th…'
complete -c clang -l print-supported-extensions -d 'Prints the list of all extensions that are supported for every CPU target for…'
...
complete -c clang -o fkeep-system-includes -d 'Usable only with \\%-E'
