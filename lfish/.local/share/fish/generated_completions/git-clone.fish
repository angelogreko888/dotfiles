...
complete -c git-clone -s r -d '<src> <dst> while modifying <src>'
complete -c git-clone -l auto -d 'git-maintenance(1)'
complete -c git-clone -s a -d 'to copy all objects from the source repository into a pack in the cloned repo…'
...
complete -c git-clone -l init
complete -c git-clone -l recursive -d '<pathspec> immediately after the clone is finished'
