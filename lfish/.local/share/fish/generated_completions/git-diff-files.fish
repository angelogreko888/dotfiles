...
complete -c git-diff-files -l dirstat-by-file -d 'Synonym for --dirstat=files,<param>,'
...
complete -c git-diff-files -l name-only -d 'Show only the name of each changed file in the post-image tree'
complete -c git-diff-files -l name-status -d 'Show only the name(s) and status of each changed file'
...
complete -c git-diff-files -o 'S<string>' -d 'Look for differences that change the number of occurrences of the specified <…'
...
complete -c git-diff-files -l inter-hunk-context -d 'Show the context between diff hunks, up to the specified <number> of lines, t…'
...
complete -c git-diff-files -l ignore-submodules -d 'Ignore changes to submodules in the diff generation.  all is the default'
...
complete -c git-diff-files -l ita-invisible-in-index -d 'By default entries added by git add -N appear as an existing empty file in gi…'
...
complete -c git-diff-files -l stat-graph-width -d 'or by setting diff. statGraphWidth=<graph-width>.  Using'
...
complete -c git-diff-files -s x -d 'for adding or removing executable bit respectively) in diffstat'
complete -c git-diff-files -s X -l dirstat
complete -c git-diff-files -l '*stat' -d files
complete -c git-diff-files -l color-moved
complete -c git-diff-files -l color-moved-ws
complete -c git-diff-files -l word-diff
complete -c git-diff-files -o 'B/70%'
complete -c git-diff-files -o 'B20%'
complete -c git-diff-files -o 'M90%'
...
complete -c git-diff-files -o 'S<regex>'
complete -c git-diff-files -s G -d 'git log'
complete -c git-diff-files -s t -d 'option in git-log to also find trees'
complete -c git-diff-files -o O/dev/null
complete -c git-diff-files -s N -d 'appear as an existing empty file in git diff and a new file in git diff'
complete -c git-diff-files -l cached -d 'git diff and non-existent in git diff'
...
complete -c git-diff-files -o p1 -d 'index header: @@@ <from-file-range> <from-file-range> <to-file-range> @@@ The…'
