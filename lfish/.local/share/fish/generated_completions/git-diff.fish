...
complete -c git-diff -l dirstat-by-file -d 'Synonym for --dirstat=files,<param>,'
...
complete -c git-diff -l name-only -d 'Show only the name of each changed file in the post-image tree'
complete -c git-diff -l name-status -d 'Show only the name(s) and status of each changed file'
...
complete -c git-diff -o 'S<string>' -d 'Look for differences that change the number of occurrences of the specified <…'
...
complete -c git-diff -l inter-hunk-context -d 'Show the context between diff hunks, up to the specified <number> of lines, t…'
...
complete -c git-diff -l ignore-submodules -d 'Ignore changes to submodules in the diff generation.  all is the default'
...
complete -c git-diff -l ita-invisible-in-index -d 'By default entries added by git add -N appear as an existing empty file in gi…'
complete -c git-diff -s 1 -l base -s 2 -l ours -s 3 -l theirs -d 'Compare the working tree with . sp'
...
complete -c git-diff -l staged -d 'is a synonym of'
complete -c git-diff -l cached
complete -c git-diff -l merge-base -d 'is given, instead of using <commit>, use the merge base of <commit> and HEAD'
complete -c git-diff -l diff-algorithm
complete -c git-diff -l stat-graph-width -d 'or by setting diff. statGraphWidth=<graph-width>.  Using'
complete -c git-diff -l stat-width
complete -c git-diff -l stat-name-width -d and
complete -c git-diff -l stat-count
complete -c git-diff -s x -d 'for adding or removing executable bit respectively) in diffstat'
complete -c git-diff -s X -l dirstat
complete -c git-diff -l '*stat' -d files
complete -c git-diff -l color-moved
complete -c git-diff -l color-moved-ws
complete -c git-diff -l word-diff
complete -c git-diff -o 'B/70%'
complete -c git-diff -o 'B20%'
complete -c git-diff -o 'M90%'
...
complete -c git-diff -o 'S<regex>'
complete -c git-diff -s G -d 'git log'
complete -c git-diff -s t -d 'option in git-log to also find trees'
complete -c git-diff -o O/dev/null
complete -c git-diff -s N -d 'appear as an existing empty file in git diff and a new file in git diff'
...
complete -c git-diff -o p1 -d 'index header: @@@ <from-file-range> <from-file-range> <to-file-range> @@@ The…'
complete -c git-diff -l refresh -d 'to update the cached stat information for paths whose contents in the work tr…'
complete -c git-diff -s U -d 'option.  diff. interHunkContext'
complete -c git-diff -s O -d 'option for details.  If diff'
