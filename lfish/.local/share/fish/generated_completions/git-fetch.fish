...
complete -c git-fetch -l all -d 'Fetch all remotes, except for the ones that has the remote. <name>'
...
complete -c git-fetch -s f -l force -d 'When git fetch is used with <src>:<dst> refspec, it may refuse to update the …'
...
complete -c git-fetch -l auto -d 'at the end to perform automatic repository maintenance if needed'
...
complete -c git-fetch -l no-all -d 'or by explicitly specifying one or more remote(s) to fetch from'
complete -c git-fetch -l split -d 'option, most executions will create a very small commit-graph file on top of …'
complete -c git-fetch -l graph -d 'fetch. bundleURI'
complete -c git-fetch -l bundle-uri -d 'option behaves in git-clone(1).  git clone'
