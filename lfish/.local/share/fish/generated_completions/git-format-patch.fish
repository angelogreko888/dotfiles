...
complete -c git-format-patch -l dirstat-by-file -d 'Synonym for --dirstat=files,<param>,'
...
complete -c git-format-patch -l inter-hunk-context -d 'Show the context between diff hunks, up to the specified <number> of lines, t…'
...
complete -c git-format-patch -l ignore-submodules -d 'Ignore changes to submodules in the diff generation.  all is the default'
...
complete -c git-format-patch -l ita-invisible-in-index -d 'By default entries added by git add -N appear as an existing empty file in gi…'
...
complete -c git-format-patch -l rfc -d 'Prepends the string <rfc> (defaults to "RFC") to the subject prefix'
...
complete -c git-format-patch -l stat-graph-width -d 'or by setting diff. statGraphWidth=<graph-width>.  Using'
...
complete -c git-format-patch -s x -d 'for adding or removing executable bit respectively) in diffstat'
complete -c git-format-patch -s X -l dirstat
complete -c git-format-patch -l '*stat' -d files
complete -c git-format-patch -o 'B/70%'
complete -c git-format-patch -o 'B20%'
complete -c git-format-patch -o 'M90%'
...
complete -c git-format-patch -l cached -d 'git diff and non-existent in git diff'
...
complete -c git-format-patch -s 3 -d 'feature/v2)'
