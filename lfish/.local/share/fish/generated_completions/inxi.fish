...
complete -c inxi -s b -l basic -d 'Show basic report with the following items: System (-S); Machine  (-M); Batte…'
...
complete -c inxi -s C -l cpu -d 'Show full CPU report (if each item available): basic CPU topology, model, typ…'
...
complete -c inxi -s e -l expanded -d 'Show expanded -b report.  Includes all Upper Case options (except '
complete -c inxi -s J -l swap -s s -s n -d 'verbose options such as -d -f -i -J -l -m -o -p -r -t -u -x  unless you use t…'
complete -c inxi -s R
...
complete -c inxi -l ip-limit -l limit -s 1 -d 'Raise or lower max report limit of IP addresses for -i.  -1  removes limit'
...
complete -c inxi -s m -l memory -d 'Memory (RAM) data.  Does not display with -b or -e unless you use '
...
complete -c inxi -l partitions-sort -l ps -d '[dev-base|fs|id|label|percent-used|size|uuid|used] Change default sort order …'
...
complete -c inxi -l sensors -d 'Show report from sensors if sensors installed/configured: Motherboard/CPU/GPU…'
...
complete -c inxi -l uuid -d 'Show UUIDs'
complete -c inxi -s U -l update -d 'Note - Maintainer may have disabled this function'
complete -c inxi -l no-ssl
complete -c inxi -s v -l verbosity -d 'Report verbosity levels.  If no verbosity level number is given, 0 is assumed'
complete -c inxi -l version -l vf -d 'inxi full version and license information.  Prints information then exits'
complete -c inxi -l version-short -l vs -d 'inxi single line version information'
complete -c inxi -s w -l weather -d 'DO NOT USE THIS FEATURE FOR AUTOMATED WEATHER UPDATES! Automated or excessive…'
...
complete -c inxi -l weather-unit -l wu -d '[m|i|mi|im] Sets weather units to metric (m),  imperial (i), metric (imperial…'
...
complete -c inxi -l filter-all -d 'Shortcut to trigger -z, --zl, --zu, --zv.  All the filters, that is'
complete -c inxi -l zl -d 'Filter partition label names from -j, -o, -p, -P,  and -Sa (root=LABEL=. )'
complete -c inxi -l zu -d 'Filter partition UUIDs from -j, -o, -p, -P, '
complete -c inxi -o Sa -o Mxxx -d cases
...
complete -c inxi -l separator -l sep -d 'Change the default report key: value separator : to something else'
...
complete -c inxi -l display -d '(with:).  If none found, shows nothing'
...
complete -c inxi -l bt-tool -d 'See --force [tool name].  Used to set -E report tool'
complete -c inxi -l dig -d 'Shortcut.  See --force dig'
...
complete -c inxi -l egl -d 'Shortcut.  See --force egl'
...
complete -c inxi -l html-wan -d 'Shortcut.  See --force --html-wan'
complete -c inxi -l ifconfig -d 'Shortcut.  See --force ifconfig'
complete -c inxi -l man -d 'Shortcut.  See --force man'
complete -c inxi -l no-dig -d 'Shortcut.  See --force no-dig'
complete -c inxi -l no-doas -d 'Shortcut.  See --force no-doas'
complete -c inxi -l no-egl -d 'Shortcut.  See --force no-egl'
complete -c inxi -l no-html-wan -d 'Shortcut.  See --force no-html-wan'
complete -c inxi -l no-man -d 'Shortcut.  See --force no-man'
complete -c inxi -l no-opengl -d 'Shortcut.  See --force no-opengl'
complete -c inxi -l no-sensor-force -d 'Overrides user set SENSOR_FORCE configuration value'
complete -c inxi -l no-sudo -d 'Shortcut.  See --force no-sudo'
...
complete -c inxi -l sensors-use -d 'Linux only.  Use only the (comma separated) sensor arrays for -s report'
...
complete -c inxi -l partition-sort
