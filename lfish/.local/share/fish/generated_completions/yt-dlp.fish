# yt-dlp
# Autogenerated from man page /usr/share/man/man1/yt-dlp.1.gz
complete -c yt-dlp -s h -l help -d 'Print this help text and exit'
complete -c yt-dlp -l version -d 'Print program version and exit'
complete -c yt-dlp -s U -l update -d 'Update this program to the latest version'
complete -c yt-dlp -l no-update -d 'Do not check for updates (default)'
complete -c yt-dlp -l update-to -d 'Upgrade/downgrade to a specific version.  CHANNEL can be a repository as well'
...
complete -c yt-dlp -l use-extractors -d 'Extractor names to use separated by commas'
complete -c yt-dlp -l default-search -d 'Use this prefix for unqualified URLs.  E. g'
...
complete -c yt-dlp -l color -d 'Whether to emit color codes in output, optionally prefixed by the STREAM (std…'
complete -c yt-dlp -l compat-options -d 'Options that can help keep compatibility with youtube-dl or youtube-dlc confi…'
complete -c yt-dlp -l alias -d 'Create aliases for an option string'
...
complete -c yt-dlp -l impersonate -d 'Client to impersonate for requests.  E. g'
complete -c yt-dlp -l list-impersonate-targets -d 'List available clients to impersonate'
...
complete -c yt-dlp -l enable-file-urls -d 'Enable file:// URLs.  This is disabled by default for security reasons'
complete -c yt-dlp -l geo-verification-proxy -d 'Use this proxy to verify the IP address for some geo-restricted sites'
complete -c yt-dlp -l xff -d 'How to fake X-Forwarded-For HTTP header to try bypassing geographic restricti…'
complete -c yt-dlp -s I -l playlist-items -d 'Comma separated playlist_index of the items to download'
complete -c yt-dlp -l min-filesize -d 'Abort download if filesize is smaller than SIZE, e. g.  50k or 44. 6M'
complete -c yt-dlp -l max-filesize -d 'Abort download if filesize is larger than SIZE, e. g.  50k or 44. 6M'
...
complete -c yt-dlp -l no-match-filters -d 'Do not use any --match-filters (default)'
complete -c yt-dlp -l break-match-filters -d 'Same as \\[dq]--match-filters\\[dq] but stops the download process when a video…'
...
complete -c yt-dlp -l retry-sleep -d 'Time to sleep between retries in seconds (optionally) prefixed by the type of…'
complete -c yt-dlp -l skip-unavailable-fragments -d 'Skip unavailable fragments for DASH, hlsnative and ISM downloads (default) (A…'
complete -c yt-dlp -l abort-on-unavailable-fragments -d 'Abort download if a fragment is unavailable (Alias: --no-skip-unavailable-fra…'
...
complete -c yt-dlp -l no-lazy-playlist -d 'Process videos in the playlist only after the entire playlist is parsed (defa…'
...
complete -c yt-dlp -l download-sections -d 'Download only chapters that match the regular expression'
...
complete -c yt-dlp -l output-na-placeholder -d 'Placeholder for unavailable fields in --output (default: \\[dq]NA\\[dq])'
...
complete -c yt-dlp -l no-windows-filenames -d 'Sanitize filenames only minimally'
...
complete -c yt-dlp -l cookies-from-browser -d 'The name of the browser to load cookies from'
complete -c yt-dlp -l no-cookies-from-browser -d 'Do not load cookies from browser (default)'
complete -c yt-dlp -l cache-dir -d 'Location in the filesystem where yt-dlp can store some downloaded information…'
...
complete -c yt-dlp -l no-quiet -d 'Deactivate quiet mode.  (Default)'
...
complete -c yt-dlp -l progress-delta -d 'Time between progress output (default: 0)'
...
complete -c yt-dlp -l add-headers -d 'Specify a custom HTTP header and its value, separated by a colon \\[dq]:\\[dq]'
...
complete -c yt-dlp -l merge-output-format -d 'Containers that may be used when merging formats, separated by \\[dq]/\\[dq], e'
...
complete -c yt-dlp -l netrc-cmd -d 'Command to execute to get the credentials for an extractor'
complete -c yt-dlp -l video-password -d 'Video-specific password'
...
complete -c yt-dlp -l client-certificate -d 'Path to client certificate file in PEM format.  May include the private key'
complete -c yt-dlp -l client-certificate-key -d 'Path to private key file for client certificate'
complete -c yt-dlp -l client-certificate-password -d 'Password for client certificate private key, if encrypted'
complete -c yt-dlp -s x -l extract-audio -d 'Convert video files to audio-only files (requires ffmpeg and ffprobe)'
complete -c yt-dlp -l audio-format -d 'Format to convert the audio to when -x is used'
complete -c yt-dlp -l audio-quality -d 'Specify ffmpeg audio quality to use when converting the audio with -x'
complete -c yt-dlp -l remux-video -d 'Remux the video into another container if necessary (currently supported: avi…'
complete -c yt-dlp -l recode-video -d 'Re-encode the video into another format if necessary'
...
complete -c yt-dlp -l exec -d 'Execute a command, optionally prefixed with when to execute it, separated by …'
complete -c yt-dlp -l no-exec -d 'Remove any previously defined --exec'
complete -c yt-dlp -l convert-subs -d 'Convert the subtitles to another format (currently supported: ass, lrc, srt, …'
...
complete -c yt-dlp -s X -d 'creates options [dq]--get-audio[dq] and [dq]-X[dq] that takes an argument (AR…'
complete -c yt-dlp -l no-skip-unavailable-fragments
...
complete -c yt-dlp -o preview -d 'https://wiki. sponsor. ajay. app/w/Segment_Categories'
complete -c yt-dlp -l no-ignore-dynamic-mpd
complete -c yt-dlp -l extractor-args -d 'Pass ARGS arguments to the IE_KEY extractor'
complete -c yt-dlp -l config-location -d '[bu] 2 If unset, ${XDG_CONFIG_HOME} defaults to [ti]/'
...
complete -c yt-dlp -l match-title
...
complete -c yt-dlp -s 1
complete -c yt-dlp -l no-playlist-reverse
complete -c yt-dlp -l no-colors -d 'Not recommended'
complete -c yt-dlp -l force-generic-extractor
...
complete -c yt-dlp -l youtube-include-hls-manifest -l no-youtube-skip-hls-manifest
complete -c yt-dlp -l geo-bypass
complete -c yt-dlp -l no-geo-bypass
complete -c yt-dlp -l geo-bypass-country
complete -c yt-dlp -l geo-bypass-ip-block -d 'Developer options'
complete -c yt-dlp -l test
complete -c yt-dlp -l load-pages
...
complete -c yt-dlp -l no-write-annotations
complete -c yt-dlp -s A -l auto-number
complete -c yt-dlp -s t -s l -l title -l literal -d CONTRIBUTING
